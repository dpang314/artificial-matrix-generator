...
main(int argc, char **argv)
{
	...
	char mtx_filename[256];
	snprintf(mtx_filename, sizeof(mtx_filename), "%s.mtx", matrix_name);
	FILE *mtx_file = fopen(mtx_filename, "w");
	if (mtx_file) {
		fprintf(mtx_file, "%%%%MatrixMarket matrix coordinate real general\n");
		// fprintf(mtx_file, "%% Generated by artificial_matrix_generator\n");
		fprintf(mtx_file, "%d %d %d\n", csr->nr_rows, csr->nr_cols, csr->nr_nzeros);
		for (unsigned int row = 0; row < csr->nr_rows; ++row) {
			for (int idx = csr->row_ptr[row]; idx < csr->row_ptr[row + 1]; ++idx) {
				int col = csr->col_ind[idx];
				double val = csr->values[idx];
				fprintf(mtx_file, "%d %d %.16g\n", row + 1, col + 1, val);
			}
		}
		fclose(mtx_file);
	} else {
